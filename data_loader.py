...
class CMEDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pot_rels = np.zeros(self.config.rel_num, dtype=int)
        ...
        if not self.is_test:
            ...
            for triple in ins_json_data['triple_list']:
                if not pot_rels[self.rel2id[triple[1]]]:
                  pot_rels[self.rel2id[triple[1]]] = 1
            ...
            if s2ro_map:
                ...
                return token_ids, masks, text_len, sub_heads, sub_tails, sub_head, sub_tail, obj_heads, obj_tails, ins_json_data['triple_list'], tokens, pot_rels
            else:
                return None
        else:
            token_ids, segment_ids = self.tokenizer.encode(first=text)
            ...
            sub_head, sub_tail = np.zeros(text_len), np.zeros(text_len)
            obj_heads, obj_tails = np.zeros((text_len, self.config.rel_num)), np.zeros((text_len, self.config.rel_num))
            pot_rels = np.zeros(self.config.rel_num)
            return token_ids, masks, text_len, sub_heads, sub_tails, sub_head, sub_tail, obj_heads, obj_tails, ins_json_data['triple_list'], tokens, pot_rels
...
def cmed_collate_fn(batch):
    ...
    token_ids, masks, text_len, sub_heads, sub_tails, sub_head, sub_tail, obj_heads, obj_tails, triples, tokens, pot_rels = zip(*batch)
    cur_batch = len(batch)
    max_text_len = max(text_len)
    rel_num = len(pot_rels[0])
    ...
    batch_pot_rels = torch.Tensor(cur_batch, rel_num).zero_()
    ...
    for i in range(cur_batch):
        ...
        batch_pot_rels[i, :].copy_(torch.from_numpy(pot_rels[i]))
